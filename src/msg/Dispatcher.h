...
public:
  ...
  /**
   * This function will be called synchronously whenever a new Connection is made
   * to the Messenger, if you support fast dispatch. It is guaranteed to be
   * called before any messages are dispatched.
   *
   * @param con The new Connection which has been established. You are not
   * granted a reference to it -- take one if you need one!
   */
