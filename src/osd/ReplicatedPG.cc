...
void ReplicatedPG::do_request_op_fast(const OpRequestRef& op)
{
  ...
  /*MOSDOp *m = static_cast<MOSDOp*>(op->get_req());
  if (osd->osd->returnDummy(m))
  {
        dout(10) << "Dummy data sent returning from ms_dispatch()" << dendl;
        //m->put();
        return ;
  }*/
...
void ReplicatedPG::check_blacklisted_obc_watchers(const ObjectContextRef& obc)
{
						     ...
						     SnapSetContext *ssc)
{
  ...
  ObjectContextRef obc(object_contexts.create_item(oi.soid));
						  ...
						  map<string, bufferptr> *attrs, bool io_path )
{
  ...
  /*map<hobject_t, ObjectContextRef>::iterator p = object_contexts_som.find(soid);
  
  if (p != object_contexts_som.end()) 
  {
    //printf("Got obc in the som_cache..\n");
    return p->second;
  }*/
  int fd;
  string fullPath;
  
  ObjectContextRef obc = object_contexts.lookup(soid); //ObjectContextRef();// = object_contexts.lookup(soid);
  //ObjectContextRef obc = ObjectContextRef();
  //dout(0) << "soid.hash = " <<  soid.hash << "soid.oid.name = " << soid.oid.name.c_str()<< "soid.snapid = " << soid.snap.val << dendl;
  if (obc) {
    //printf("obc not null !!\n");
    ...
    if (attrs) {
      ...
      //printf("pgbackend->objects_get_attr \n"); 
      int r;
      if (io_path)
      //if (false)
      {
	//printf("Going for fasr attr read\n");
    ...
    //assert(oi.soid == soid);
    //printf("Before object_contexts.lookup_or_create \n");
    //obc = object_contexts.lookup_or_create(oi.soid);
    obc = object_contexts.create_item(oi.soid);
    //obc = object_contexts.create_item(soid);
    obc->destructor_callback = new C_PG_ObjectContext(this, obc.get());
    obc->obs.oi = oi;
    obc->obs.exists = true;
    ...
    populate_obc_watchers(obc);
    //object_contexts_som[oi.soid] = obc;
    //object_contexts_som.insert(make_pair(oi.soid, obc));
