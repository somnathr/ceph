			...
			FDRef& outfd)
{
  ...
  /*if (!replaying) {
    ...
    int status = fdcache.lookup(oid, outfd);
    if (status == 0)
    {
      	return 0;
    }
  ...
  if (!index_manager.isUpgrade())
  {
  	r = index_manager.get_fd_fast(cid, oid, basedir, flags, fd); 
  ...
  else
  {
	Index index;
...

  	{
    		...
    		fd = r;
    		outfd = FDRef(new FDCache::FD(fd));//make_shared<FD>(fd);

  	}
   }

  /*if (!replaying) {
    fdcache.add(oid, outfd);
  }*/
  return 0;
